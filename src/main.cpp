...
uint256 hashGenesisBlock("0x0000041d63e3ca943325071a7e581895ee6ddce1cda60b50333a5852f5e1fd4c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // magnum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "magnum Signed Message:\n";
...
{
    // magnum: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // magnum: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "mgm genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1484056839;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1055780;
        ...
        assert(block.hashMerkleRoot == uint256("0x382659346c8cecdf535a05ae8033a4b277f88713547f1413bfb834dac1303b8e"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // magnum: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("magnumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("magnumMiner : ProcessBlock, block not accepted");
    }
